...
A self-hosted timer overlay for OBS with real-time sync, multi-room support, and full customization.

## Features

- **Timer modes**: Countdown and countup
- **End behaviors**: Stop at zero, count negative, hide, or confetti celebration
- **Multi-room**: Isolated timers via `?room=xxx` URL parameter
...
| `/control?room=xxx` | Control panel |
...
2. Set URL to `http://your-domain/overlay?room=myroom`
